...
int main(){
	...
	XGpio_Initialize(&Buttons, XPAR_AXI_GPIO_SW_DEVICE_ID);

	XGpio_SetDataDirection(&Buttons, 2, 0xFFFF);
	...
	while(1){
		...
		if((XGpio_DiscreteRead(&Buttons, 2) == 1) && !prev_btn_st){
			prev_btn_st = 1;
			initialized = 0;
		...
		else if((XGpio_DiscreteRead(&Buttons, 2) == 1) && prev_btn_st){
			clearLEDs(led_num);
		}
