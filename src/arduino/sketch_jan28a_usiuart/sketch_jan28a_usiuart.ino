/*
  ...
  https://electronics.stackexchange.com/questions/241915/pcint0-interrupt-on-attiny
...
void initializeUSI () {
  ...
  USICR = 0;                      // Disable USI.
  //note: couldn't find mappings MW:  
  GIFR = 1<<PCIF;                 // Clear pin change interrupt flag.
  //interrupts();
  GIMSK |= 1<<PCIE;               // Enable pin change interrupts
  PCMSK |= 1 << PCINT5;           // Enable pin change on pin PB5
}
...
ISR (USI_OVF_vect) {
  ...
  uint8_t temp = 0xff; //this needs fixed: USIBR;
  USICR  =  0;                    // Disable USI
